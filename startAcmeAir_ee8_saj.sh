...
ACMEAIR_IMAGE_NAME="liberty-acmeair-ee8:myJ17"
...
if [[ $? -eq 0 ]]; then
  # -e MONGO_MANUAL="true"
  docker run --rm -it -v /tmp:/opt/ol/wlp/output/defaultServer/ -m=100m --cpus=4.0 --network=$NET_NAME --name acmeair -p 9080:9080 -e MONGO_HOST="mongodb" -e MONGO_PORT="27017" -e MONGO_DBNAME="acmeair" $ACMEAIR_IMAGE_NAME
  stopMongo
else
